...
def animate(i):
    ...
    ax[0].plot(x, sin_y, color='b')

    ax[1].clear()
    ax[1].set_ylim(-1.2, 1.2)
    ax[1].axhline(0, color='k')
    ax[1].plot(x, cos_y, color='r')
    ...
    ax[-1].plot(x1, y1, color='b', label='sin')
    ax[-1].set_ylabel('updates / sec')
    ax[-1].legend()


class Sin_SubHandler():
    """
...
if __name__ == "__main__":
    ...
    try:
        ...
        sin_sub = client.create_subscription(1 , sin_handler)
        sin_handle = sin_sub.subscribe_data_change(sin_var)

        cos_handler = Cos_SubHandler()
        cos_sub = client.create_subscription(1 , cos_handler)
        cos_handle = cos_sub.subscribe_data_change(cos_var)
        time.sleep(0.1)
